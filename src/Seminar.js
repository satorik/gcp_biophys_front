...
import {
  updateAfterCreate,
  updateAfterDelete,
} from './utils/updateCache/seminar'
...
import { SelectPanel } from './components/Seminar/SelectPanel'
...
const Seminar = () => {
  ...
  const {
    loading: queryLodading,
    error: queryError,
    data,
  } = useQuery(queries.GET_SEMINAR, { variables })
  const [createSeminar, { loading: creatingLoading }] = useMutation(
    queries.CREATE_SEMINAR,
    {
      update: (cache, res) =>
        updateAfterCreate(cache, res, queries.GET_SEMINAR, variables),
    }
  )

  const [updateSeminar, { loading: updatingLoading }] = useMutation(
    queries.UPDATE_SEMINAR
  )
  const [deleteSeminar, { loading: deletingLoading }] = useMutation(
    queries.DELETE_SEMINAR,
    {
      update: (cache, res) =>
        updateAfterDelete(cache, res, queries.GET_SEMINAR, variables),
    }
  )
  ...
  if (queryLodading || creatingLoading || updatingLoading || deletingLoading)
    return <Spinner />
  ...
  const onChangeSeminarHandler = async (postObject) => {
    if (mode.isEditing) {
      ...
      try {
        await updateSeminar({
          variables: { id: updatedSeminar.id, inputData: forUpdate },
        })
    ...
    if (mode.isCreating) {
      try {
        const createdSeminar = await createSeminar({
  ...
  let modalTitle = ''
  if (mode.isEditing) {
    modalTitle = 'Редактирование семинара'
  }
  if (mode.isCreating) {
    modalTitle = 'Новый семинар'
  }
  if (mode.isDeleting) {
    modalTitle = 'Удаление семинара'
  }
    ...
    <>
      {isModalOpen && (
        <Modal isOpen={isModalOpen} title={modalTitle} onClose={onCloseModal}>
          ...
          {mode.isDeleting && (
            <YesDelete
              onDelete={onDeleteSeminarHandler}
        ...
        <div className="container mt-5">
          <div className="row">
            <SelectPanel
              seminars={seminars}
              selectedId={viewId}
              onSelect={onShowSeminarDetails}
            />
            <SeminarDetails
              title={seminars[viewId].title}
              location={seminars[viewId].location}
              content={seminars[viewId].content}
              onClickEdit={() => onEditSeminar(viewId)}
              onClickDelete={() => onDeleteSeminar(viewId)}
            />
          </div>
        </div>
      )}
