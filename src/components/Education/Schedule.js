...
import {
  updateAfterCreate,
  updateAfterDelete,
  updateAfterCreateTime,
  updateAfterDeleteTime,
} from '../../utils/updateCache/schedule'
...
const DAY_TEMPLATE = [
  ...
  },
  {
    title: 'isEven',
    label: [
      { title: 'Каждую неделю', value: '0' },
      { title: 'По четным', value: '2' },
      { title: 'По нечетным', value: '1' },
    ],
    type: 'radio',
    validators: [],
  },
...
const Schedule = () => {
  ...
  const {
    ...
    error: queryError,
    data,
  } = useQuery(queries.GET_YEARS, { variables })
  const [createScheduleYear, { loading: creationLoading }] = useMutation(
    queries.CREATE_YEAR,
    {
      update: (cache, res) =>
        updateAfterCreate(cache, res, queries.GET_YEARS, variables),
    }
  )
  const [updateScheduleYear, { loading: updatingLoading }] = useMutation(
    queries.UPDATE_YEAR
  )
  const [deleteScheduleYear, { loading: deletingLoading }] = useMutation(
    queries.DELETE_YEAR,
    {
      update: (cache, res) =>
        updateAfterDelete(cache, res, queries.GET_YEARS, variables),
    }
  )
  const [
    createScheduleTimetable,
    { loading: createTimeTableLoading },
  ] = useMutation(queries.CREATE_TIMETABLE, {
    update: (cache, res) =>
      updateAfterCreateTime(cache, res, queries.GET_YEARS, variables, viewId),
  })
  const [
    updateScheduleTimetable,
    { loading: updateTimeTableLoading },
  ] = useMutation(queries.UPDATE_TIMETABLE)
  const [
    deleteScheduleTimetable,
    { loading: deleteTimeTableLoading },
  ] = useMutation(queries.DELETE_TIMETABLE, {
    update: (cache, res) =>
      updateAfterDeleteTime(cache, res, queries.GET_YEARS, variables, viewId),
  })
  ...
  const openSimpleAlert = (twoDoubles, twoEven) => {
    ...
    if (twoDoubles)
      setAlertMessage('Невозможно поставить три лекции на одно и то же время')
    if (twoEven)
      setAlertMessage(
        'Невозможно поставить две лекции по четным или две по нечетным'
      )
    setDoubleFound(true)
  }
  ...
  const onChangeTimeHandler = async postObject => {
    if (timeMode.isEditing) {
      ...
      const double = getIsDouble(
        postObject,
        timetableByDay[DAYS[updatedTime.dayId]],
        updatedTime.id
      )
      const forUpdate = getUpdateData(updatedTime, {
        ...postObject,
        isDouble: +double.isDouble,
      })
      if (
        double.isOneEven &&
        (double.isOneDouble || +double.isDouble === updatedTime.isDouble)
      ) {
        try {
          await updateScheduleTimetable({
            variables: { id: updatedTime.id, inputData: forUpdate },
          })
          if (forUpdate.isDouble !== undefined) {
            const newDouble = forUpdate.isDouble === 0 ? 0 : +updatedTime.id
            await updateScheduleTimetable({
              variables: {
                id: updatedTime.isDouble,
                inputData: { isDouble: newDouble },
              },
            })
          }
    ...
    if (timeMode.isCreating) {
      ...
      if (double.isOneDouble && double.isOneEven) {
        try {
          await createScheduleTimetable({
            variables: {
              yearId: years[viewId].id,
              dayId: DAYS.indexOf(updatedDay),
              inputData: { ...postObject, isDouble: +double.isDouble },
            },
          })
  ...
  const onChangeYearHandler = async postObject => {
    if (mode.isEditing) {
      try {
        await updateScheduleYear({
          variables: { id: updatedYear.id, inputData: postObject },
        })
        clearMode('isEditing')
      } catch (error) {
        setIsError(error)
  ...
  let modalTitle = ''
  if (mode.isEditing) {
    modalTitle = 'Редактирование расписания'
  }
  if (mode.isCreating) {
    modalTitle = 'Новое расписание'
  }
  if (mode.isDeleting) {
    modalTitle = 'Удаление расписания'
  }
  if (timeMode.isEditing) {
    modalTitle = 'Редактирование лекции'
  }
  if (timeMode.isDeleting) {
    modalTitle = 'Удаление лекции'
  }
  if (doubleFound) {
    modalTitle = 'Одновременные лекции'
  }
    ...
    <>
      {isModalOpen && (
        <Modal isOpen={isModalOpen} title={modalTitle} onClose={onCloseModal}>
          ...
          {mode.isDeleting && (
            <YesDelete
              onDelete={onDeleteScheduleYearHandler}
              onCancel={onCloseModal}
              info={updatedYear}
              instance={'educationYear'}
            />
          )}
          {timeMode.isDeleting && (
            <YesDelete
              onDelete={onDeleteScheduleTimeHandler}
              onCancel={onCloseModal}
              info={updatedTime}
              instance={'scheduleDay'}
            />
          )}
          {doubleFound && (
            <SimpleAlert alert={alertMessage} onClose={onCloseSimpleAlert} />
          )}
        </Modal>
      )}
      {data.years.length !== 0 && (
