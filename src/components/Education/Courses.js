...
import {
  updateAfterCreate,
  updateAfterDelete,
  updateAfterCreateResourse,
  updateAfterDeleteResourse,
  updateAfterUpdateResourse,
} from '../../utils/updateCache/courses'
import * as queries from '../../utils/queries/courses'

import { CourseInfo } from './CourseInfo'
import { CourseMaterials } from './CourseMaterials'
import YesDelete from '../Shared/DoYouWantToDelete'
import { ButtonAddResourse } from '../UI/ButtonAddResourse'
...
const Courses = () => {
  ...
  const [resourseMode, setResourseMode] = React.useState({
    isEditing: false,
    isCreating: false,
    isDeleting: false,
  })
  ...
  const [createEducationCourse, { loading: creationLoading }] = useMutation(
    queries.CREATE_COURSE,
    {
      update: (cache, res) =>
        updateAfterCreate(cache, res, queries.GET_COURSES),
    }
  )
  const [updateEducationCourse, { loading: updatingLoading }] = useMutation(
    queries.UPDATE_COURSE
  )
  const [deleteEducationCourse, { loading: deletingLoading }] = useMutation(
    queries.DELETE_COURSE,
    {
      update: (cache, res) =>
        updateAfterDelete(cache, res, queries.GET_COURSES),
    }
  )
  const [
    createEducationResourse,
    { loading: createResourseLoading },
  ] = useMutation(queries.CREATE_RESOURSE, {
    update: (cache, res) =>
      updateAfterCreateResourse(cache, res, queries.GET_COURSES, viewId),
  })
  const [
    updateEducationResourse,
    { loading: updateResourseLoading },
  ] = useMutation(queries.UPDATE_RESOURSE, {
    update: (cache, res) =>
      updateAfterUpdateResourse(cache, res, queries.GET_COURSES, viewId),
  })
  const [
    deleteEducationResourse,
    { loading: deleteEducationResourseLoading },
  ] = useMutation(queries.DELETE_RESOURSE, {
    update: (cache, res) =>
      updateAfterDeleteResourse(cache, res, queries.GET_COURSES, viewId),
  })
  ...
  if (
    ...
    updatingLoading ||
    deletingLoading ||
    createResourseLoading ||
    updateResourseLoading ||
    deleteEducationResourseLoading
  )
    return <Spinner />
  ...
  if (courses.length > 0) {
    ...
    singleResourses.push(
      ...courses[viewId].resourses.filter(
        resourse => resourse.form.type === 'SINGLE'
      )
    )
    multyResourses.push(
      ...courses[viewId].resourses.filter(
        resourse => resourse.form.type === 'MULTY'
      )
    )

    let materialsToAdd = forms.filter(
      form =>
        singleResourses.filter(resourse => resourse.form.id === form.id)
          .length === 0
    )
  ...
  const onDeleteResourse = id => {
    ...
    setUpdatedResourse(
      courses[viewId].resourses.find(resourse => resourse.id === id)
    )
  }
  ...
  const onChangeResourseHandler = async postObject => {
    const filetype = forms.find(form => form.id === postObject.educationFormId)
      .filetype
    ...
    if (mode.isEditing) {
      ...
      try {
        await updateEducationCourse({
          variables: { id: updatedCourse.id, inputData: forUpdate },
        })
  ...
  let modalTitle = ''
  if (mode.isEditing) {
    modalTitle = 'Редактирование курса'
  }
  if (mode.isCreating) {
    modalTitle = 'Новый курс'
  }
  if (mode.isDeleting) {
    modalTitle = 'Удаление курса'
  }
  if (resourseMode.isDeleting) {
    modalTitle = 'Удаление материалов'
  }
  if (resourseMode.isCreating) {
    modalTitle = 'Новый материал'
  }
  if (resourseMode.isEditing) {
    modalTitle = 'Редактирование материалов'
  }
    ...
    <>
      {isModalOpen && (
        <Modal isOpen={isModalOpen} title={modalTitle} onClose={onCloseModal}>
          {(mode.isCreating || mode.isEditing) && (
            <Edit
          ...
          {mode.isDeleting && (
            <YesDelete
              ...
              info={updatedCourse}
              instance="educationCourse"
            />
          )}
          {resourseMode.isDeleting && (
            <YesDelete
              onDelete={onDeleteResourseHandler}
              onCancel={onCloseModal}
              info={updatedResourse}
              instance="educationResourse"
            />
          )}
          {resourseMode.isEditing && (
            <div className="p-2">
              <Edit
          ...
          <div className="container bg-light pb-2 px-0">
            ...
            {(singleResourses.length > 0 || multyResourses.length > 0) && (
              ...
              >
                ...
                <span>
                  <FontAwesomeIcon
                    icon={showResourses.basic ? faCaretUp : faCaretDown}
                    size="2x"
                  />
                </span>
              </div>
              ...
              <>
                {(singleResourses.length > 0 || multyResourses.length > 0) && (
                  <>
                      ...
                      <div className="container">
                        <div className="row">
                          {singleResourses.map(resourse => (
                            <div className="p-2 col-md-6" key={resourse.id}>
                              <TextbookCard
                    ...
                    {multyResourses.length > 0 && (
                      <div>
                        {forms.map(form => {
                          if (form.type === 'MULTY') {
                            const typedReses = multyResourses.filter(
                              res =>
                                res.form.id === form.id ||
                                (res.form.parentForm &&
                                  res.form.parentForm.id === form.id)
                            )
            ...
            {!resourseMode.isCreating && !resourseMode.isEditing && (
              <ButtonAddResourse onClick={onAddNewResourse} />
            )}
            {resourseMode.isCreating && (
              <div className="p-2">
                <Edit
                  onClickSubmit={onChangeResourseHandler}
                  onClickCancel={onCloseModal}
      ...
      {data.courses.length === 0 && <EmptyData instance="educationCourse" />}
      <ButtonAddNew
        color="red"
