...
const Staff = () => {
  ...
  const { loading: queryLoading, error: queryError, data } = useQuery(
    queries.GET_DEPARTMENT_STAFF
  )
  const [createDepartmentPerson, { loading: creationLoading }] = useMutation(
    queries.CREATE_DEPARTMENT_STAFF,
    {
      update: (cache, res) =>
        updateAfterCreate(cache, res, queries.GET_DEPARTMENT_STAFF),
    }
  )
  const [updateDepartmentPerson, { loading: updatingLoading }] = useMutation(
    queries.UPDATE_DEPARTMENT_STAFF
  )
  const [deleteDepartmentPerson, { loading: deletingLoading }] = useMutation(
    queries.DELETE_DEPARTMENT_STAFF,
    {
      update: (cache, res) =>
        updateAfterDelete(cache, res, queries.GET_DEPARTMENT_STAFF),
    }
  )
  const [
    movePersonPosition,
    { loading: updatingPersonPositionLoading },
  ] = useMutation(queries.MOVE_PERSON, {
    update: (cache, res) =>
      updateAfterMove(cache, res, queries.GET_DEPARTMENT_STAFF),
  })
  ...
  const onChangeDepartmentStaffHandler = async postObject => {
    if (mode.isEditing) {
      ...
      try {
        await updateDepartmentPerson({
          variables: { id: updatedPerson.id, inputData: forUpdate },
        })
  ...
  let modalTitle = ''
  if (mode.isEditing) {
    modalTitle = 'Редактирование человека'
  }
  if (mode.isCreating) {
    modalTitle = 'Новый человек'
  }
  if (mode.isDeleting) {
    modalTitle = 'Удаление человека'
  }
    ...
    <>
      {isModalOpen && (
        <Modal isOpen={isModalOpen} title={modalTitle} onClose={onCloseModal}>
          ...
          {mode.isDeleting && (
            <YesDelete
              onDelete={ondeleteDepartmentPersonHandler}
