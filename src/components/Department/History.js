...
import {
  updateAfterCreate,
  updateAfterDelete,
} from '../../utils/updateCache/history'
...
const History = () => {
  ...
  const { loading: queryLodading, error: queryError, data } = useQuery(
    queries.GET_DEPARTMENT_HISTORY
  )
  const [createHistory, { loading: creationLoading }] = useMutation(
    queries.CREATE_HISTORY,
    {
      update: (cache, res) =>
        updateAfterCreate(cache, res, queries.GET_DEPARTMENT_HISTORY),
    }
  )
  const [updateHistory, { loading: updatingLoading }] = useMutation(
    queries.UPDATE_HISTORY
  )
  const [deleteHistory, { loading: deletingLoading }] = useMutation(
    queries.DELETE_HISTORY,
    {
      update: cache => updateAfterDelete(cache, queries.GET_DEPARTMENT_HISTORY),
    }
  )

  if (queryLodading || creationLoading || updatingLoading || deletingLoading)
    return <Spinner />
    ...
    <>
      {isModalOpen && (
        <Modal isOpen={isModalOpen} title={modalTitle} onClose={onCloseModal}>
          {mode.isDeleting && (
            <YesDelete
              onDelete={onDeleteHistoryHandler}
            ...
            {!mode.isEditing && !mode.isCreating && history && (
              <>
                ...
                <img
                  ...
                  className="rounded img-thumbnail mb-3 HistoryImage"
                />
                <p
                  className="p-2"
