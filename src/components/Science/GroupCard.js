...
const GroupCard = ({ group, articleForUpdate, personForUpdate }) => {
  const content = group.mode.isEditing ? (
    ...
    <>
      <img
        src={group.imageUrl}
        className="img-thumbnail float-right ml-2"
        alt="..."
      ...
      <div className="mb-2">
        {group.tel && (
          <p className="m-0">
            ...
            >
              <FontAwesomeIcon icon="phone" size="sm" /> {group.tel}
            </a>
          </p>
        )}
        {group.room && (
          <p className="m-0" style={{ color: '#4818a0' }}>
            <FontAwesomeIcon icon="home" size="sm" /> {group.room}
          </p>
        )}
      </div>
      <p
    ...
    <div className="p-3">
      ...
      <div>
        {group.articles.data.map((article, idx) => {
          if (
            group.articles.mode.isEditing &&
            article.id === articleForUpdate.id
          ) {
            return (
              <Edit
                key={article.id}
        ...
        {group.articles.mode.isCreating && (
          <div className="p-3">
            <h3>Публикация</h3>
            <Edit
              onClickSubmit={group.articles.onChangeArticle}
