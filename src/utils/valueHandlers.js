...
export const getIntitialValue = (
  ...
  if (isForUpdate) {
    ...
    if (type === 'resourse') {
      return {
        educationFormId: oldData.form.parentForm
          ? oldData.form.parentForm.id
          : oldData.form.id,
  ...
  if (type === 'course') {
    return { course: '', year: new Date().getFullYear(), term: 1 }
  }
  if (type === 'resourse') {
    return {
      educationFormId: label[0].id,
      file: '',
      subSectionId: '',
      subSectionText: '',
    }
